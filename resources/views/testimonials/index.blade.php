...
@section('content')
    ...
    <!-- Header Section -->
    <div class="bg-gradient-to-r from-pink-600 to-orange-500 text-white">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        ...
                        <p class="text-white/80 mt-1">Suara dari komunitas Maravia</p>
                    </div>
                </div>
                ...
                <div class="hidden md:flex space-x-6">
                    <div class="text-center">
                        ...
                        <div class="text-xs text-white/80">Total Testimoni</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold">{{ $testimonials->count() }}</div>
                        <div class="text-xs text-white/80">Ditampilkan</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold">
                            <i class="fas fa-star"></i>
                        </div>
                        <div class="text-xs text-white/80">Terverifikasi</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-100 dark:border-gray-700">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <button onclick="refreshTestimonials()" class="text-primary-600 hover:text-primary-700 font-semibold text-sm">
                            Refresh →
                        </button>
                    </div>
                    ...
                    @forelse($testimonials as $item)
                        <div class="group bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 transition-all duration-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer mb-4" onclick="showTestimonialDetail({{ $item->id }})">
                            <div class="flex items-start space-x-4">
                                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-pink-500 to-orange-500 flex items-center justify-center text-white flex-shrink-0">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <blockquote class="mt-3 text-gray-700 dark:text-gray-200 leading-relaxed">
                                        <span class="excerpt">{{ Str::limit($item->message, 200) }}</span>
                                        @if(strlen($item->message) > 200)
                                            <span class="full hidden">{{ $item->message }}</span>
                                            <button type="button" class="toggle-more text-blue-600 dark:text-blue-400 cursor-pointer font-medium ml-1">Selengkapnya</button>
                                        @endif
                                    </blockquote>
                                    <div class="mt-3 flex items-center space-x-3 text-xs text-gray-500 dark:text-gray-400">
                                        <span class="inline-flex items-center bg-pink-50 text-pink-700 dark:bg-pink-900/40 dark:text-pink-300 px-2 py-1 rounded-full">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-100 dark:border-gray-700">
                    ...
                    <form method="POST" action="{{ route('testimonials.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="message" rows="4" required placeholder="Tulis pengalaman Anda menggunakan Maravia..."
                        ...
                        <button type="submit" class="w-full bg-primary-600 hover:bg-primary-700 text-white py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center">
...
<script>
    ...
    function showTestimonialDetail(id) {
        // Placeholder: could navigate to a dedicated detail page if available
    }

    function shareTestimonials() {
        if (navigator.share) {
            navigator.share({
                title: 'Testimoni Maravia',
                text: 'Lihat testimoni dari komunitas Maravia',
    ...
    // Expand/collapse testimonial message
    document.addEventListener('click', function(e){
        const btn = e.target.closest('.toggle-more');
        if (!btn) return;
        e.preventDefault();
        const block = btn.closest('blockquote');
        if (!block) return;
        const excerpt = block.querySelector('.excerpt');
