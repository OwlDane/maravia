...
<div class="admin-page space-y-8">
    ...
    <div class="admin-header">
        <div class="px-6 py-8">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                <div class="space-y-2">
                    <h1 class="text-2xl lg:text-3xl font-bold">Edit Category</h1>
                    <p class="text-gray-600 text-sm lg:text-base">Update this category's information</p>
                </div>
    ...
    <div class="px-6 pb-8">
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="max-w-4xl mx-auto">
            @csrf
            @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-4">
                                    <input type="color" id="color" name="color" value="{{ old('color', $category->color ?? '#f97316') }}"
                                           class="w-16 h-12 border border-gray-200 rounded-xl cursor-pointer">
                                    <div class="flex-1">
                                        <input type="text" id="color-text" value="{{ old('color', $category->color ?? '#f97316') }}"
                                ...
                                <div class="grid grid-cols-6 gap-3 bg-amber-50 rounded-xl p-4 max-h-40 overflow-y-auto">
                                    ...
                                    @foreach($icons as $icon)
                                        <label class="flex items-center justify-center w-12 h-12 bg-white rounded-lg border-2 border-transparent hover:border-amber-300 cursor-pointer transition-colors">
                                            <input type="radio" name="icon" value="{{ $icon }}" class="hidden" {{ old('icon', $category->icon ?? 'fas fa-folder') === $icon ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Sync color inputs
    colorInput.addEventListener('change', function() { colorTextInput.value = this.value; });
    ...
    iconRadios.forEach(radio => {
        ...
        radio.addEventListener('change', function() {
            iconRadios.forEach(r => r.closest('label').classList.remove('border-amber-400', 'bg-amber-100'));
            if (this.checked) { label.classList.add('border-amber-400', 'bg-amber-100'); }
        });
        if (radio.checked) { label.classList.add('border-amber-400', 'bg-amber-100'); }
    });
});
</script>
@endpush
